...
import { products, wantedItems } from '../../data/mock'
...
<template>
  <main class="page-wrap bazaar-page">
    ...
    <el-tabs v-model="activeTab" class="bazaar-tabs" @tab-change="changeTab">
      <el-tab-pane label="求购广场" name="wanted">
        ...
        <div class="wanted-grid">
          <el-card v-for="item in wantedItems" :key="item.title" shadow="hover">
            <h3>{{ item.title }}</h3>
      ...
      <el-tab-pane label="以物换物" name="swap">
        ...
        <div class="product-grid">
          <ProductGridCard v-for="product in products.filter((item) => item.swap)" :key="product.id" :product="product" />
        </div>
      </el-tab-pane>
      ...
      <el-tab-pane label="毕业季专题" name="season">
        ...
        <div class="product-grid">
          <ProductGridCard v-for="product in products.slice(0, 6)" :key="product.id" :product="product" />
        </div>
      </el-tab-pane>
    </el-tabs>
  </main>
